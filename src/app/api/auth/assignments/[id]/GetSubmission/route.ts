...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    var assignmentSubmission = await repo.findOne({
      where: {
    ...
    if (!assignmentSubmission) {
      return NextResponse.json(ResponseFactory.error("No assignmentSubmission found for the given assignment id","NOT_FOUND"),{status: 404});
    }

    let assignmentSubmissionDTO = new AssignmentSubmissionAnswerDto(assignmentSubmission);

    return NextResponse.json(ResponseFactory.success(assignmentSubmissionDTO),{status: 200 });
