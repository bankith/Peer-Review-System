...
import { PeerReviewSubmissionSubmitCommentAndScoreDto } from "@/dtos/PeerReview/PeerReviewSubmission/PeerReviewSubmissionSubmitCommentAndScoreDto";
...
export async function POST(req: NextRequest, { params }: { params: { peerReviewSubmissionId: string } }) {
  try {
    ...
    const dto = plainToInstance(PeerReviewSubmissionSubmitCommentAndScoreDto, body);  
    ...
    const peerReviewSubmission = await AppDataSource.manager.findOneOrFail(PeerReviewSubmission, {
      where: {
        id: submissionId
      },
    });

    peerReviewSubmission.reviewScore = dto.reviewScore;
    peerReviewSubmission.updatedBy = jwt.userId;    
    ...
    const saveResult = await AppDataSource.manager.save(peerReviewSubmission);
    const saveResult2 = await AppDataSource.manager.save(peerReviewComment);
            
    return NextResponse.json(ResponseFactory.success({saveResult, saveResult2}), {status: 200});
