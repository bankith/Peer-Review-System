...
const PeerReviewSubmissionReview = ({ peerReview, peerReviewSubmission, comments, reviewrNumber, isAnswerSectionEnable = true, reviwerName, reviwerGroupName, isReviewee }: PeerReviewSubmissionReviewProps) => {    
  ...
  var isUseScoreText = false;
  if(peerReview?.peerReviewType == PeerReviewTypeEnum.Text || comments.length == 0){
    isUseScoreText = true;
  }
  return (
    <>     
      <div className="bg-white px-6 py-5 mt-6 shadow dark:bg-dark-1 rounded-lg">
        ...
        <div className="text-3xl text-dark font-bold mt-10 text-center align-middle">{isUseScoreText ? '-' : peerReviewSubmission?.reviewScore}/10</div>
        <div className="col-span-4">
          ...
          {(peerReviewSubmission && isAnswerSectionEnable) ? <AnswerCommentSection peerReviewSubmission={peerReviewSubmission} canScore={comments.length == 0 ? true : false} isGrading={false} peerReviewSubmissionId={peerReviewSubmission.id} peerReview={peerReview} /> : null}
          </>
          }
        </div>
